笔记：
npm run serve 执行后 直接调用 main.js
vue : vue 核心 + 模板解析器
...
1. 功能：可以把多个组件共用的配置提取成一个混入对象
   如果混合中有 X，vm 中也有 X，则以 vm 中的为主（data、methods）
   生命周期不是，是来者不拒
2. 使用方式：
   ...
   第二步使用混入：
    ...
    全局混入：`Vue.mixin(xxx)`，vm 和所有的 vc 都可以用
   ​ 局部混入：`mixins:['xxx'] `
...
1. 作用：让样式在局部生效，防止冲突。
2. 写法：`<style scoped>` 这样各自的样式就不会发生冲突
...
1. 组件化编码流程：
    ...
    (2).实现动态组件：考虑好数据的存放位置，数据是一个组件在用，还是一些组件在用（读/写）：
...
1. 一种组件间通信的方式，适用于：<strong style="color:red">子组件 ===> 父组件</strong>
   ...
   为什么：因为实在父组件中给子组件绑定自定义事件，说明已经是一种父子关系了
  ...
  devServer: {
    proxy: {
      "/api1": {
        ...
        changeOrigin: true,
        // 当加上前缀后，请求的资源将会变成/apil/students，而不是原来那个
        // 所以，需要这个配置将apil变为空
        pathRewrite: { "^/api1": "" },
      },
      "/api2": {
